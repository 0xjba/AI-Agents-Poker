        ...
        try:
            ...
            if player_state.stack <= 100:
                ...
                if player_state.stack >= min_raise:
                    ...
                    await self.make_action(3, player_state.stack)  # Action 3 is RAISE
                # Case 2: If we can't raise but there's a bet to call, do a call all-in
                elif call_amount > 0:
                    logger.info(f"Using CALL for all-in (stack: {player_state.stack}, call_amount: {call_amount})")
                    await self.make_action(2)  # Action 2 is CALL
                # Case 3: No bet to call, use check (can add chips to pot later)
                else:
            ...
            if remaining_after_call <= 100 and remaining_after_call > 0:
                ...
                if amount_to_call == 0:
                    ...
                    if can_raise:
                        ...
                        # If we can't raise but can check, just check
                        logger.info("Cannot raise all-in, checking instead")
                        action_type = 1  # CHECK
                        self.action_reasoning = "Checking as we cannot raise all-in but have minimal stack"
                        await self.make_action(action_type)
                    else:
                        # Fallback to fold if we can't raise or check
                        logger.warning("Cannot raise all-in or check, folding as fallback")
                        action_type = 0  # FOLD
                        self.action_reasoning = "Folding as we cannot go all-in or check"
                        await self.make_action(action_type)
                else:
                    # There's a bet to call
                    if can_raise and player_state.stack > amount_to_call:
                        # We can raise and have chips beyond the call amount
                        action_type = 3  # RAISE
                        ...
                        # Fallback to fold if we can't raise or call
                        logger.warning("Cannot raise or call all-in, folding as fallback")
                        action_type = 0  # FOLD
